...
struct VerletObject
{
    ...
    void flipVelocityX(float dt) {
        sf::Vector2f v = getVelocity(dt);
        v.x = -v.x;
        setVelocity(v, dt);
    }

    void flipVelocityY(float dt) {
        sf::Vector2f v = getVelocity(dt);
        v.y = -v.y;
        setVelocity(v, dt);
    }
...
public:
    ...
    void setAllObjectsVelocity(sf::Vector2f v) {
...
private:
    ...
    void applyConstraint(float dt)
    {
        for (auto& obj : m_objects) {
            ...
            if (obj.position.x > size.x - obj.radius) {
                obj.position.x = size.x - obj.radius;
                obj.flipVelocityX(dt);
            }
            ...
            if (obj.position.y > size.y - obj.radius) {
                obj.position.y = size.y - obj.radius;
                obj.flipVelocityY(dt);
            }
